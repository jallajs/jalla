...
function script (entry, uri, app) {
  ...
  if (watch) {
    ...
    var watcher = watchify.emitter(b)
    watcher.on('update', function (buff, rows) {
      app.emit('bundle:script', entry, `${uri}.js`, buff)
    })
    watcher.on('pending', function (files) {
  ...
  return get(new RegExp(route), function middleware (ctx, bundle, type = 'js') {
    ...
    if (watch) {
      return new Promise(function (resolve, reject) {
        watcher.middleware(ctx.req, ctx.res)
  ...
  // emit progress on pipeline reset
  // () -> void
  function onreset () {
    app.emit('progress', entry, `${uri}.js`, 0)
  }

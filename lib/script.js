...
function script (entry, uri, app) {
  ...
  b.on('split.pipeline', function (pipeline, entry, name) {
    branches[name] = { entry: entry }
    app.emit('progress', entry, name, 0)
  })
  ...
  //                   ↓ hash     ↓ uri   ↓ branch            ↓ type
  var route = `^\\/(?:(?:\\w+\\/)?${uri})(?:-([-\\w]+))?.*\\.(?=(js|map))[^.]*$`
  return get(new RegExp(route), function middleware (ctx, branch, type) {
    ctx.type = `application/${type === 'map' ? 'json' : 'javascript'}`
    ctx.set('Cache-Control', `max-age=${watch ? 0 : 60 * 60 * 24 * 365}`)

    if (branch) {
      if (type === 'map') ctx.body = branches[`${uri}-${branch}.js`].map
      else ctx.body = branches[`${uri}-${branch}.js`].buffer
      return
    }
  ...
  function bundleDynamicBundle (bundleName) {
    var stream = concat({encoding: 'buffer'}, function (buff) {
      app.emit('bundle:script', branches[bundleName].entry, bundleName, buff)
      var hash
      if (watch) hash = 'dev'
      else hash = crypto.createHash('sha512').update(buff).digest('buffer').toString('hex').slice(0, 16)
      stream.emit('name', `${hash}/${bundleName}`)
      branches[bundleName].buffer = buff
    })
    return stream
  }

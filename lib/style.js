...
function style (entry, uri, app) {
  ...
  var config = postcssrc(ctx, basedir)
    ...
    .catch(() => ({ plugins: [atImport, copyAssets()], options: ctx }))
    .then(function (result) {
      if (watch) result.plugins.push(watcher.plugin())
      else result.plugins.push(autoprefixer, csso)
      return result
    })
  ...
  app.on('bundle:file', function (file) {
    ...
    deps.push(new Promise(function (resolve, reject) {
      resolvePath(path.dirname(file), opts, function (err, result) {
        if (!err && result) app.emit('bundle:file', result)
  ...
  app.on('progress', function (file) {
    if (file === app.entry) deps = []
  })

  app.on('bundle:script', async function (file) {
    if (file !== app.entry) return
    var prev = processing
    processing = Promise.all(deps).then(async function (resolved) {
      resolved = resolved.filter(Boolean)
      if (!difference(resolved, include)) {
        if (!resolved.length) {
          // emit an empty bundle if no files were found
          app.emit('bundle:style', entry || app.entry, `${uri}.css`, Buffer.from(NO_CONTENT))
        }
        return prev
      }
      include = resolved
      if (entry) include.unshift(entry)
      var { options } = await config
  ...
  return get(new RegExp(route), async function middleware (ctx, type = 'css') {
    ...
    var maxage = type === 'map' || watch ? 0 : 60 * 60 * 24 * 365
  ...
  async function process (options) {
    app.emit('progress', entry || app.entry, `${uri}.css`, 0)
    try {
      let processor = await bundle
      let content = include.map((file) => `@import "${file}";`).join('\n')
      ...
      app.emit('bundle:style', entry || app.entry, `${uri}.css`, buffer)
  ...
  function copyAssets () {
    ...
    function onasset (asset, dir, opts) {
      ...
      var name = asset.relativePath
      app.emit('progress', asset.absolutePath, name, 0)
      var buff = fs.readFileSync(asset.absolutePath)
      app.emit('bundle:asset', asset.absolutePath, name, buff)
      return app.context.assets[name].url
    }
  }
}

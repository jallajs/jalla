...
module.exports = class App extends Koa {
  constructor (entry, opts) {
    ...
    var self = this
    var restricted = new Set()
    if (opts.sw) restricted.add(opts.sw)
    ...
    function onbundle (file, uri, buff) {
      ...
      if (!restricted.has(file) && self.env !== 'development') {
    ...
    // add registered asset to list of restricted (non-hashable) assets
    // (str, str, Buffer) -> void

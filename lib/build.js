...
function ssr (state, emit) {
  var b = browserify(state.entry, {
    ...
    fullPaths: true,
    standalone: path.basename(CLIENT_NAME, '.js')
  })
  ...
  if (state.env !== 'development') {
    ...
    if (parseInt(version.major, 10) >= 12) {
      // common-shakeify tends to throw a "heap out of memory" error in old node
      b.plugin(shakeify)
    }
  }

  b.on('reset', function restart () {
    emit('progress', CLIENT_NAME, 0)
  })

  return function bundle (cb) {
    var map = path.resolve(state.dist, CLIENT_NAME + '.map')
    var ssr = new Promise(function (resolve, reject) {
      var client = path.resolve(state.dist, CLIENT_NAME)
      b.bundle().on('error', reject)
        ...
        .pipe(fs.createWriteStream(client, 'utf8'))
        .on('close', resolve)
    })
  ...
  function template () {
    return new Promise(function (resolve, reject) {
      ...
      resolveFile('.', { basedir: dir, extensions: ['.html'] }, function (err, file) {

...
function document (html, state, app) {
  ...
  if (app.env !== 'development') {
    ...
    if (app.serve) {
      // TODO: implement asset read method
      let url = app.context.assets.get('bundle.css').url.replace(/^\//, '')
      let css = path.resolve(app.dist, url)
      d.transform(inlineCriticalCSS(css))
    } else {
      let css = app.context.assets.get('bundle.css').buffer.toString()
      let critical = dropcss({ html, css })
      d.transform(addToHead, `<style>${critical.css}</style>`)
    }

    let features = process.env.POLYFILL_FEATURES
    d.transform(addToHead, `
      <link rel="preload" as="style" href="${app.context.assets.get('bundle.css').url}" onload="this.rel='stylesheet'">
...
// str -> Stream
function inlineCriticalCSS (file) {
  var html = ''

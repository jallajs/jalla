var ora = require('ora')

var SPINNER = {
  interval: 120,
  frames: ['⬘', '⬗', '⬙', '⬖']
}

var STATUS = /([\w-]+):(asset|progress)$/

module.exports = ui

function ui (app) {
  var bundling = new Set()
  var spinner = ora({
    text: 'Initializing',
    spinner: SPINNER,
    color: 'reset'
  }).start()

  app.on('error', function (err) {
    spinner.stopAndPersist({
      symbol: '◆',
      text: (app.env === 'development' && err.stack) || err.message || err
    })
  })

  app.on('warning', function (warning) {
    spinner.stopAndPersist({ symbol: '▵', text: warning })
  })

  app.pipeline.on('*', function (event, id) {
    var match = event.match(STATUS)
    if (!match) return
    var [, label, name] = match
    if (name === 'progress') {
      bundling.add(label)
    } else if (name === 'asset') {
      bundling.delete(label)
      spinner.stopAndPersist({ symbol: '◆', text: id })
    }
    if (bundling.size) spinner.start(Array.from(bundling).join(', '))
  })

  app.on('timing', function (time, ctx) {
    var url = ctx.url
    var prefix = `${ctx.method} ${ctx.status} `
    var suffix = ` [${time} ms]`
    var space = process.stdout.columns - (prefix.length + suffix.length)

    if (url.length > space) {
      // try fit in stdout by trimming each path segment
      url = url.split('/').map(function (segment) {
        return segment.length > 16 ? segment.substr(0, 16) + '…' : segment
      }).join('/')
    }

    if (url.length > space) {
      // just truncate the whole thing if still too long
      url = url.substr(0, space - 3).replace(/\/?…?$/, '…')
    }

    spinner.stopAndPersist({ symbol: '◀︎', text: prefix + url + suffix })
  })

  app.on('start', function (port) {
    if (app.env === 'development') {
      spinner.stopAndPersist({
        symbol: '◆',
        text: `Server listening at http://localhost:${port}`
      })
    } else {
      spinner.stopAndPersist({
        symbol: '◆',
        text: `Server listening on port ${port}`
      })
    }
    if (bundling.size) spinner.start(Array.from(bundling).join(', '))
  })
}
